#!/usr/bin/env node

/**
 * Luxembourgish Data Protection MCP — stdio entry point.
 *
 * Provides MCP tools for querying CNPD decisions, sanctions, and
 * data protection guidance documents.
 *
 * Tool prefix: lu_dp_
 */

import { Server } from "@modelcontextprotocol/sdk/server/index.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import {
  CallToolRequestSchema,
  ListToolsRequestSchema,
} from "@modelcontextprotocol/sdk/types.js";
import { readFileSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { z } from "zod";
import {
  searchDecisions,
  getDecision,
  searchGuidelines,
  getGuideline,
  listTopics,
} from "./db.js";
import { buildCitation } from "./utils/citation.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

let pkgVersion = "0.1.0";
try {
  const pkg = JSON.parse(
    readFileSync(join(__dirname, "..", "package.json"), "utf8"),
  ) as { version: string };
  pkgVersion = pkg.version;
} catch {
  // fallback to default
}

const SERVER_NAME = "luxembourgish-data-protection-mcp";

// --- Tool definitions ---------------------------------------------------------

const TOOLS = [
  {
    name: "lu_dp_search_decisions",
    description:
      "Full-text search across CNPD decisions (délibérations, sanctions, mises en demeure). Returns matching decisions with reference, entity name, fine amount, and GDPR articles cited.",
    inputSchema: {
      type: "object" as const,
      properties: {
        query: {
          type: "string",
          description: "Search query in French (e.g., 'Amazon', 'sous-traitant', 'transfert données', 'consentement')",
        },
        type: {
          type: "string",
          enum: ["sanction", "mise_en_demeure", "deliberation", "avis"],
          description: "Filter by decision type. Optional.",
        },
        topic: {
          type: "string",
          description: "Filter by topic ID (e.g., 'sous_traitance', 'cookies', 'transferts'). Optional.",
        },
        limit: {
          type: "number",
          description: "Maximum number of results to return. Defaults to 20.",
        },
      },
      required: ["query"],
    },
  },
  {
    name: "lu_dp_get_decision",
    description:
      "Get a specific CNPD decision by reference number (e.g., 'CNPD-2021-001', 'CNPD-DEC-2023-001').",
    inputSchema: {
      type: "object" as const,
      properties: {
        reference: {
          type: "string",
          description: "CNPD decision reference (e.g., 'CNPD-2021-001', 'CNPD-DEC-2023-001')",
        },
      },
      required: ["reference"],
    },
  },
  {
    name: "lu_dp_search_guidelines",
    description:
      "Search CNPD guidance documents: guides pratiques, recommandations on vidéosurveillance, cookies, sous-traitance, transferts, and financial sector obligations.",
    inputSchema: {
      type: "object" as const,
      properties: {
        query: {
          type: "string",
          description: "Search query in French (e.g., 'vidéosurveillance', 'sous-traitant', 'analyse impact')",
        },
        type: {
          type: "string",
          enum: ["guide", "recommandation", "information", "FAQ"],
          description: "Filter by guidance type. Optional.",
        },
        topic: {
          type: "string",
          description: "Filter by topic ID (e.g., 'sous_traitance', 'vidéosurveillance', 'cookies'). Optional.",
        },
        limit: {
          type: "number",
          description: "Maximum number of results to return. Defaults to 20.",
        },
      },
      required: ["query"],
    },
  },
  {
    name: "lu_dp_get_guideline",
    description:
      "Get a specific CNPD guidance document by its database ID.",
    inputSchema: {
      type: "object" as const,
      properties: {
        id: {
          type: "number",
          description: "Guideline database ID (from lu_dp_search_guidelines results)",
        },
      },
      required: ["id"],
    },
  },
  {
    name: "lu_dp_list_topics",
    description:
      "List all covered data protection topics with French and English names. Use topic IDs to filter decisions and guidelines.",
    inputSchema: {
      type: "object" as const,
      properties: {},
      required: [],
    },
  },
  {
    name: "lu_dp_about",
    description: "Return metadata about this MCP server: version, data source, coverage, and tool list.",
    inputSchema: {
      type: "object" as const,
      properties: {},
      required: [],
    },
  },
];

// --- Zod schemas for argument validation --------------------------------------

const SearchDecisionsArgs = z.object({
  query: z.string().min(1),
  type: z.enum(["sanction", "mise_en_demeure", "deliberation", "avis"]).optional(),
  topic: z.string().optional(),
  limit: z.number().int().positive().max(100).optional(),
});

const GetDecisionArgs = z.object({
  reference: z.string().min(1),
});

const SearchGuidelinesArgs = z.object({
  query: z.string().min(1),
  type: z.enum(["guide", "recommandation", "information", "FAQ"]).optional(),
  topic: z.string().optional(),
  limit: z.number().int().positive().max(100).optional(),
});

const GetGuidelineArgs = z.object({
  id: z.number().int().positive(),
});

// --- Helper ------------------------------------------------------------------

function textContent(data: unknown) {
  return {
    content: [
      { type: "text" as const, text: JSON.stringify(data, null, 2) },
    ],
  };
}

function errorContent(message: string) {
  return {
    content: [{ type: "text" as const, text: message }],
    isError: true as const,
  };
}

// --- Server setup ------------------------------------------------------------

const server = new Server(
  { name: SERVER_NAME, version: pkgVersion },
  { capabilities: { tools: {} } },
);

server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: TOOLS,
}));

server.setRequestHandler(CallToolRequestSchema, async (request) => {
  const { name, arguments: args = {} } = request.params;

  try {
    switch (name) {
      case "lu_dp_search_decisions": {
        const parsed = SearchDecisionsArgs.parse(args);
        const results = searchDecisions({
          query: parsed.query,
          type: parsed.type,
          topic: parsed.topic,
          limit: parsed.limit,
        });
        return textContent({ results, count: results.length });
      }

      case "lu_dp_get_decision": {
        const parsed = GetDecisionArgs.parse(args);
        const decision = getDecision(parsed.reference);
        if (!decision) {
          return errorContent(`Decision not found: ${parsed.reference}`);
        }
        const d = decision as unknown as Record<string, unknown>;
        return textContent({
          ...decision,
          _citation: buildCitation(
            String(d.reference || parsed.reference),
            String(d.title || d.reference || parsed.reference),
            "lu_dp_get_decision",
            { reference: parsed.reference },
            d.source_url as string | undefined,
          ),
        });
      }

      case "lu_dp_search_guidelines": {
        const parsed = SearchGuidelinesArgs.parse(args);
        const results = searchGuidelines({
          query: parsed.query,
          type: parsed.type,
          topic: parsed.topic,
          limit: parsed.limit,
        });
        return textContent({ results, count: results.length });
      }

      case "lu_dp_get_guideline": {
        const parsed = GetGuidelineArgs.parse(args);
        const guideline = getGuideline(parsed.id);
        if (!guideline) {
          return errorContent(`Guideline not found: id=${parsed.id}`);
        }
        const g = guideline as unknown as Record<string, unknown>;
        return textContent({
          ...guideline,
          _citation: buildCitation(
            String(g.reference || g.title || `guideline-${parsed.id}`),
            String(g.title || g.reference || `Guideline ${parsed.id}`),
            "lu_dp_get_guideline",
            { id: String(parsed.id) },
            g.source_url as string | undefined,
          ),
        });
      }

      case "lu_dp_list_topics": {
        const topics = listTopics();
        return textContent({ topics, count: topics.length });
      }

      case "lu_dp_about": {
        return textContent({
          name: SERVER_NAME,
          version: pkgVersion,
          description:
            "CNPD (Commission Nationale pour la Protection des Données) MCP server. Provides access to Luxembourgish data protection authority decisions, sanctions, and official guidance documents on GDPR enforcement in Luxembourg.",
          data_source: "CNPD (https://cnpd.public.lu/)",
          coverage: {
            decisions: "CNPD délibérations, sanctions, and mises en demeure (incl. Amazon EUR 746M)",
            guidelines: "CNPD guides on vidéosurveillance, cookies, sous-traitance, transferts, secteur financier",
            topics: "Sous-traitance, vidéosurveillance, cookies, transferts, consentement, analyse impact, droit d'accès, violation de données, secteur financier",
          },
          tools: TOOLS.map((t) => ({ name: t.name, description: t.description })),
        });
      }

      default:
        return errorContent(`Unknown tool: ${name}`);
    }
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return errorContent(`Error executing ${name}: ${message}`);
  }
});

// --- Main --------------------------------------------------------------------

async function main(): Promise<void> {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  process.stderr.write(`${SERVER_NAME} v${pkgVersion} running on stdio\n`);
}

main().catch((err) => {
  process.stderr.write(`Fatal error: ${err instanceof Error ? err.message : String(err)}\n`);
  process.exit(1);
});
